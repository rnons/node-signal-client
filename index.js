...
class App extends MatrixPuppetBridgeBase {
  ...
  handleSignalMessage(payload, message, timeStamp, members = []) {
    this.handleTypingEvent(payload.roomId, false, payload.roomId); // stop typing status if message received
//       if (members.length > 1) {       //Group
//         members.filter(phoneNumber => !phoneNumber.match(this.myNumber));
//       }
      window.log.info("members:",members);
    if ( message.body ) {
